...
class TestPublicViews:
    ...
    def test_events_view_search_and_time_filtering(self, client, db):
        ...
        e_rock = Event.objects.create(name="Rock Concert", start_datetime=now + timedelta(days=5))
        e_jazz = Event.objects.create(name="Jazz Night", start_datetime=now + timedelta(days=5))
        e_past = Event.objects.create(name="Old Rock", start_datetime=now - timedelta(days=2))
...
class TestPurchaseViews:
    ...
    def test_payment_success_view_requires_login(self, client, pending_order):
        url = reverse(f'payment_success', kwargs={'order_id': pending_order.id})
        response = client.get(url)

        assert response.status_code == 302
        assert '/login/' in response.url

    def test_payment_success_view_for_logged_user(self, client, test_user, pending_order):
    ...
    def test_cart_view_get_with_items(self, client, test_user, reserved_ticket, pending_order):
        client.force_login(test_user)
        reserved_ticket.reserved_until = timezone.now() + timedelta(minutes=10)
        reserved_ticket.save()

        OrderDetails.objects.create(order=pending_order, participant=None, ticket=reserved_ticket)
        url = reverse('cart')
        response = client.get(url)
        assert response.status_code == 200
        assert response.context['total'] == Decimal("99.99")
        assert 'expiry_time' in response.context
        assert response.context['expiry_time'] == reserved_ticket.reserved_until.isoformat()
    ...
    def test_tickets_view_no_tickets_selected_shows_error(self, client, test_user, future_event):
        client.force_login(test_user)
    ...
    @patch('tickets.views.reserve_tickets')
    def test_tickets_view_catches_validation_error(self, mock_reserve, client, test_user, future_event):
        client.force_login(test_user)
        future_event.has_numbered_seats = True
        future_event.save()
    ...
    def test_cart_view_clears_expired_cart_on_get(self, client, test_user, reserved_ticket, pending_order):
        client.force_login(test_user)
    ...
    def test_finalize_cart_payment_success(self, mock_stripe_retrieve, mock_finalize_order, client, test_user,
                                           pending_order):
        client.force_login(test_user)
        ...
        assert response.status_code == 302
        assert response.url == reverse('payment_success', kwargs={'order_id': pending_order.id})
...
class TestDashboardViews:
    ...
    def test_my_orders_view_calculates_total_price(self, client, test_user, future_event):
        client.force_login(test_user)

        order = Order.objects.create(user=test_user, status=Order.Status.COMPLETED)
        t1 = Ticket.objects.create(event=future_event, sector="A", row="1", seat="1", price=Decimal("100.00"),
                                   status=Ticket.Status.SOLD)
        t2 = Ticket.objects.create(event=future_event, sector="A", row="1", seat="2", price=Decimal("50.00"),
                                   status=Ticket.Status.SOLD)
    ...
    def test_order_details_view_can_cancel_logic(self, client, test_user, future_event):
        client.force_login(test_user)
        order = Order.objects.create(user=test_user, status=Order.Status.COMPLETED)
        t = Ticket.objects.create(event=future_event, sector="A", row="1", seat="1", price=Decimal("10.00"),
                                  status=Ticket.Status.SOLD)
    ...
    def test_cancel_order_post_only_and_calls_service(self, mock_cancel_service, client, test_user, pending_order):
        client.force_login(test_user)
...
class TestScannerViews:
    ...
    def test_scan_ticket_view_masks_pesel_on_get(self, client, staff_user, future_event):
        client.force_login(staff_user)
        order = Order.objects.create(user=staff_user, status=Order.Status.COMPLETED)
        t = Ticket.objects.create(event=future_event, sector="A", row="1", seat="1", price=Decimal("10.00"),
                                  status=Ticket.Status.SOLD)
        participant = Participant.objects.create(user=staff_user, first_name="Jan", last_name="Kowalski",
                                                 pesel="12345678901")

        detail = OrderDetails.objects.create(order=order, ticket=t, participant=participant)
    ...
    def test_scan_ticket_view_post_updates_status(self, client, staff_user, future_event):
        client.force_login(staff_user)
        order = Order.objects.create(user=staff_user, status=Order.Status.COMPLETED)
        t = Ticket.objects.create(event=future_event, sector="A", row="1", seat="1", price=Decimal("10.00"),
                                  status=Ticket.Status.SOLD)
        participant = Participant.objects.create(user=staff_user, first_name="Jan", pesel="12345678901")
        detail = OrderDetails.objects.create(order=order, ticket=t, participant=participant)
    ...
    def test_scan_ticket_view_post_already_scanned(self, client, staff_user, future_event):
        client.force_login(staff_user)
        order = Order.objects.create(user=staff_user, status=Order.Status.COMPLETED)
        t = Ticket.objects.create(event=future_event, sector="A", row="1", seat="1", price=Decimal("10.00"),
