...
class TestOrderLogic:
    ...
    @pytest.mark.django_db
    def test_reserve_tickets_refreshes_existing_timer(
        self, test_user, create_tickets, pending_order
    ):
    ...
    @pytest.mark.django_db
    def test_reserve_tickets_lazy_cleanup_on_expired_cart(
        self, test_user, create_tickets, pending_order
    ):
        tickets = create_tickets(2)
    ...
    def test_release_order_tickets_releases_and_deletes(
        self, test_user, create_tickets, pending_order
    ):
    ...
    def test_update_participants_mixed_existing_and_new(
        self, test_user, reserved_ticket, pending_order, future_event
    ):
        ...
        p1 = Participant.objects.create(
            user=test_user, first_name="Michael", last_name="Brown"
        )
        detail_1 = OrderDetails.objects.create(
            order=pending_order, ticket=reserved_ticket, participant=p1
        )
    ...
    def test_update_participants_missing_pesel_raises_error(
        self, test_user, reserved_ticket, pending_order
    ):
        reserved_ticket.event.need_pesel = True
        reserved_ticket.event.save()

        detail = OrderDetails.objects.create(
            order=pending_order, ticket=reserved_ticket
        )
    ...
    @pytest.mark.django_db
    def test_finalize_order_happy_path(
        self, mock_pdf_task, reserved_ticket, pending_order
    ):
        detail = OrderDetails.objects.create(
            order=pending_order, ticket=reserved_ticket
        )
    ...
    def test_finalize_order_ignores_non_pending_orders(self, pending_order):
        ...
        with patch(
            "tickets.services.order_logic.generate_tickets_pdf_task.delay_on_commit"
        ) as mock_task:
            finalize_order(pending_order.id)
    ...
    def test_cancel_order_fails_if_event_started(self, test_user, pending_order):
        ...
        with pytest.raises(
            ValidationError,
            match="Cannot cancel: The event 'Old Event' has already started or passed.",
        ):
            cancel_order_service(test_user, pending_order.id)
