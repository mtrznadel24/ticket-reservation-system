...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--amount", type=int, help="Amount of tickets to create without numbered seats")
    ...
    def handle(self, *args, **options):
        ...
        if event.has_numbered_seats:
            ...
            if not all([sectors, rows, seats]):
                raise CommandError("For events with numbered seats, you must provide --sectors, --rows, and --seats")
            ...
            for sector in sectors:
                for row in range(1, rows + 1):
                    for seat in range(1, seats + 1):
                        tickets.append(Ticket(event=event, price=price, sector=sector, row=str(row), seat=str(seat)))

        else:
            amount = options["amount"]

            if amount is None:
                raise CommandError("For events with no numbered seat,you must specify an amount of tickets to create with --amount")
